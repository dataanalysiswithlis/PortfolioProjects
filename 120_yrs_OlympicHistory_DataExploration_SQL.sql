...
with highest_lowest_participating_countries as
(
select *, dense_rank() over(order by re.total_countries desc) max_rank, dense_rank() over(order by re.total_countries) min_rank
from (
select oe.games, count(distinct onr.region) as total_countries
from olympic_events oe
join olympic_noc_regions onr 
...
--15. SQL query to fetch the top 10 countries who won medals in olympics.
with t1 as(
...
with t1 as(select onr.region as country,count(case when medal='Gold' then 1  end)as Gold_medal,
                            count(case when medal='Silver' then 1  end)as Silver_medal,
                            count(case when medal='Bronze' then 1 end)as Bronze_meda
from olympic_events oe
join olympic_noc_regions onr 
on oe.noc=onr.noc
group by onr.region
