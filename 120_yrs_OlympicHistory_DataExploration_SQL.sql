...
with sport_per_games as(select sport,games 
		                from  olympic_events
						where season='Summer'
						group by sport,games
 						order by games,sport),
 		
games_count as(select sport,count(games) as total_games
				from sport_per_games
 				group by sport),
 	
total_summer_games as(select count(distinct games) as total_games_in_summer 
						from olympic_events
			 			where season='Summer')
select *
 from games_count
...
total_winter_games as(select count(distinct games) as total_games_in_winter 
		 				from olympic_events
	 					where season='Winter')
select *
	from games_count
...
with t1 as(select sex as female,count(sex) as female_athletes
        	from olympic_events
       	 	where sex='F'
       		 group by sex),
       		 
t2 as(select sex as male, count(sex) as male_athelets
        from olympic_events
        where sex='M'
        group by sex
)
...
with t1 as(select name,team,count(name) as total_gold_medals
	from olympic_events
	where medal='Gold'
	group by name,team
	order by total_gold_medals desc),
	
t2 as(select *,dense_rank()over (order by total_gold_medals desc) as "top_5_rank"
    from t1
)
...
with t1 as(select name,team,count(name) as total_gold_medals, dense_rank()over (order by count(name) desc) as "medal_rank"
        from olympic_events
        where medal='Gold'
        group by name,team)
...
with t1 as(select name,team,count(name) as total_medals
	from olympic_events
	where medal in('Gold','Silver','Bronze')
	group by name,team
order by total_medals desc),

  t2 as(select *,dense_rank() over(order by total_medals desc)as "r"
      	from t1)
...
with t1 as(select onr.region ,count(medal) as total_medals
	from olympic_events oe
	join olympic_noc_regions onr 
	on oe.noc=onr.noc
	where medal in ('Gold','Silver','Bronze')
...
t2 as (select *,dense_rank() over (order by total_medals desc) as "rank"
	    from t1)
...
with event_country_medal as (select distinct oe.event "event", onr.region as country, oe.medal as medal
        from olympic_events oe
        join olympic_noc_regions onr 
        on oe.noc=onr.noc and oe.medal != 'NA'
